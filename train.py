...
tf.app.flags.DEFINE_integer('input_size', cfgs.img_size, '')
tf.app.flags.DEFINE_integer('batch_size_per_gpu', cfgs.batch_size, '')
tf.app.flags.DEFINE_integer('num_readers', 16, '')
tf.app.flags.DEFINE_float('learn_rate_init', 0.0001, '')
tf.app.flags.DEFINE_float('learn_rate_end', 0.000001, '')
tf.app.flags.DEFINE_integer('max_steps', 1000000, '')
tf.app.flags.DEFINE_float('moving_average_decay', 0.999, '')
tf.app.flags.DEFINE_string('gpu_list', '0', '')
tf.app.flags.DEFINE_string('checkpoint_path',
                           '/home/jyyu/models/cocoapi-master/model_files/polarmask/checkpoint_path/', '')
...
def tower_loss(images, input_labels, input_targets, input_mask_targets, input_mask_masks, input_mask_centers,
               reuse_variables=None):
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables):
        cls_out, target_out, mask_target_out, centerness_out = model.model(images, res_type="res18", is_training=True)
    ...
    pred_per_img_draw = draw_pred_tf(box_cls_flatten[0], box_regression_flatten[0], centerness_flatten[0], images[0])
    pred_per_img_draw1 = draw_pred_tf(box_cls_flatten[1], box_regression_flatten[1], centerness_flatten[1], images[1])
    pred_per_img_draw2 = draw_pred_tf(box_cls_flatten[2], box_regression_flatten[2], centerness_flatten[2], images[2])
    ...
    return total_loss, model_loss, boxesloss, cls_loss, center_loss, mask_loss
...
def main(argv=None):
    ...
    input_images = tf.placeholder(tf.float32, shape=[FLAGS.batch_size_per_gpu, FLAGS.input_size, FLAGS.input_size, 3],
                                  name='input_images')
    input_labels = tf.placeholder(tf.float32, shape=[FLAGS.batch_size_per_gpu, cfgs.new_number_levels],
                                  name='input_labels')
    input_targets = tf.placeholder(tf.float32, shape=[FLAGS.batch_size_per_gpu, cfgs.new_number_levels, 4],
                                   name='input_targets')
    input_mask_targets = tf.placeholder(tf.float32, shape=[FLAGS.batch_size_per_gpu, cfgs.new_number_levels, 36],
                                        name='input_mask_targets')
    input_mask_masks = tf.placeholder(tf.float32, shape=[FLAGS.batch_size_per_gpu, cfgs.new_number_levels, 36],
                                        name='input_mask_masks')
    input_mask_centers = tf.placeholder(tf.float32,
                                        shape=[FLAGS.batch_size_per_gpu * len(gpus), cfgs.new_number_levels, 1],
                                        name='input_mask_centers')
    global_step = tf.get_variable('global_step', [], initializer=tf.constant_initializer(0), trainable=False)
    # learning_rate = tf.train.exponential_decay(FLAGS.learning_rate, global_step, decay_steps=1000, decay_rate=0.98,
    #                                            staircase=True)
    ...
    learning_rate = tf.cond(
        ...
        true_fn=lambda: global_step / warmup_steps * FLAGS.learn_rate_init,
    ...
    total_loss, model_loss, boxes_loss, cls_loss, center_loss, mask_loss = \
        tower_loss(input_images, input_labels, input_targets, input_mask_targets, input_mask_masks, input_mask_centers,
                   reuse_variables)
    ...
    if FLAGS.pretrained_model_path is not None:
        ...
        variable_restore_op = slim.assign_from_checkpoint_fn(model_path, slim.get_trainable_variables(),
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for step in range(FLAGS.max_steps):
            ...
            ml, tl, bl, cl, cel, mal, _ = sess.run([model_loss, total_loss, boxes_loss, cls_loss, center_loss,
                                                    mask_loss, train_op], feed_dict={input_images: data[0],
                                                                                     input_labels: data[2],
            ...
            if step % 1 == 0:
                ...
                start = time.time()
                print('Step {:06d}, model loss {:.4f}, total loss {:.4f}, box loss {:.4f}, cls loss {:.4f}, '
                      'center loss {:.4f} mask loss {:.4f} {:.2f} seconds/step, '
                      '{:.2f} examples/second'.format(step, ml, tl, bl, cl, cel, mal,
